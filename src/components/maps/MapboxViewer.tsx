...
const MapboxViewer: React.FC<MapboxViewerProps> = ({
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      map.current.addLayer({
        ...
        paint: {
          'heatmap-weight': ['interpolate', ['linear'], ['get', 'footprint'], 1e-7, 0.1, 0.8, 1],
          'heatmap-intensity': ['interpolate', ['linear'], ['zoom'], 3, 1, 8, 0.5],
          'heatmap-color': [
            'interpolate',
            ['linear'],
            ['heatmap-density'],
            ...
            'rgba(255, 219, 213, 0)',
            0.1,
            colors.footprintScale[0], // Lightest
            0.3,
            ...
            1,
            colors.footprintScale[5], // Darkest
          ],
          'heatmap-radius': ['interpolate', ['linear'], ['zoom'], 3, 15, 8, 5],
      ...
      map.current.addLayer({
        ...
        'source-layer': 'location_-111-9dkxy7',
        paint: {
          'circle-radius': ['interpolate', ['linear'], ['zoom'], 3, 25, 5, 20, 8, 15, 12, 12],
          'circle-color': [
            'interpolate',
            ['linear'],
            ['get', 'pm25'],
      ...
      map.current.on('mousemove', e => {
        ...
        hoverPopup.current?.setLngLat(e.lngLat).setHTML(popupContent).addTo(map.current);
      });
      ...
      map.current.on('click', 'footprint-layer', e => {
        ...
        if (selectedLocation) {
          ...
          if (selectedLocation.footprint === footprint && selectedLocation.pm25 === pm25) {
  ...
  useEffect(() => {
    ...
    if (selectedLocation) {
      ...
      try {
        ...
        map.current.setFilter('footprint-layer', [
          '>',
          ['get', 'footprint'],
  ...
  const adjustThreshold = (type: 'increase' | 'decrease') => {
    if (layerType === 'footprint') {
      const newThreshold =
        type === 'increase' ? minFootprintThreshold * 2 : minFootprintThreshold / 2;
      ...
      const newThreshold = type === 'increase' ? minPm25Threshold * 2 : minPm25Threshold / 2;
    ...
    >
      ...
      >
        ...
        <p style={{ margin: '5px 0 0 0' }}>
          • Click a point to select it and view detailed information
        </p>
